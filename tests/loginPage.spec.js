...
test("Login page has email input", async ({ page }) => {
  ...
  const emailInput = page
    .locator("input[type='email'], input[name*='email' i], #email")
    .first();
...
test("Login page has password input", async ({ page }) => {
  ...
  const passwordInput = page
    .locator("input[type='password'], input[name*='password' i], #password")
    .first();
...
test("Login page has submit button", async ({ page }) => {
  ...
  const button = page
    .locator(
      "button[type='submit'], button:has-text(/(bejelentkezés|login|submit)/i)",
    )
    .first();
...
test("Login page has register option", async ({ page }) => {
  ...
  const registerLink = page
    .locator(
      "button:has-text(/(regisztráció|register|sign up)/i), a:has-text(/(regisztráció|register|sign up)/i)",
    )
    .first();
