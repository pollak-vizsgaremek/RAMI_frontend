...
test("Leaderboard page displays title or heading", async({page}) => {
    ...
    const leaderboardHeading = page.getByRole("heading", { name: /rangsor|leaderboard|top|ranglista/i });
...
test("Leaderboard shows medal or trophy icons", async({page}) => {
    ...
    const trophyIcons = page.locator('svg[data-icon*="trophy"], [class*="trophy"]');
...
test("Leaderboard displays instructor locations", async({page}) => {
    ...
    const locationIcons = page.locator('svg[data-icon*="map"], [class*="location"]');
